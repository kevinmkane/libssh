...
int ssh_client_hykex_init(ssh_session session)
{
    ...
    rc = ssh_buffer_pack(hybrid_pubkey,
                         "PB",
                         session->next_crypto->oqs_pk, session->next_crypto->oqs_kem->length_public_key,
                         pubkey);
    if (rc < 0) {
...
static SSH_PACKET_CALLBACK(ssh_packet_client_hykex_reply) {
    ...
    session->next_crypto->ecdh_server_pubkey = ssh_string_new(ssh_string_len(session->next_crypto->oqs_remote_msg) - session->next_crypto->oqs_kem->length_public_key);
    ...
    /* PQ data no longer required at this point. */
